...
class Plot:
    ...
    """
    def plotSamples(self,X,k=10000,mx=None,mn=None):
        r,s = X.simulate(k,output=True)
        counts = Counter(r)
        x = []
        y = []
        for val in counts:
        ...
        if X.isDiscrete():
            ...
            return
            #TODO: add plotting for continuous RVs (bucketing of some form)
        m = {
            'xlabel' : f'Value of {X}',
            'ylabel': f'P[X = x]',
    ...
    def plotGeneric(self,data=None,fInfo=None,output=False,chart=None,wait=False,addStats=False,onlyMean=False):
        ...
        if chart:
            if addStats:
                ...
                print(f'm={m},b={b}')
            self.fillInChartInfo(chart,wx=.9,wy=3.7) #TODO: fix weights/centering of text on charts
        if not wait:
            self._showPlt()
